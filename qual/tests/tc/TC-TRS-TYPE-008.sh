tc_TRS_TYPE_008() {
    local F="$1"; local B="$F/TC-TRS-TYPE-008"
    ...
    printf '%s' "$exp" | grep -qF '"type": "UseCase"' \
        && pass "UseCase recognised in export" || fail "UseCase not found at declared type in export"
