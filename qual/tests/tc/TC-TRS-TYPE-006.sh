tc_TRS_TYPE_006() {
    local F="$1"; local B="$F/TC-TRS-TYPE-006"
    ...
    printf '%s' "$exp" | grep -qF '"type": "AnalysisCase"' \
        && pass "AnalysisCase recognised in export" || fail "AnalysisCase not found at declared type in export"
