...
id: REQ-TRS-TYPE-009
type: Requirement
name: "Tool shall recognise and validate the AllocationDef element"
status: draft
reqDomain: software
...
- **SysMLv2 mapping:** `AllocationDef` maps to `allocation def` (§2.1) and classes an allocation relationship. Type-specific fields are defined in §8.13.1: `supertype:`, `ends:` (connection-end schema, §8.4.2), and `allocations:` (nested sub-allocation usages with `allocatedFrom:`/`allocatedTo:`).
