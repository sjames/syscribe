...
**Acceptance criteria:** A `Requirement` with `id: REQ-TRS-001` passes; `id: REQ-001` (no category segment) also passes, since the category segment(s) before the numeric suffix are optional; `id: REQ-trs-001` (lowercase) and `id: REQTRS001` (no hyphens) each produce `E006`.
