//! Relocate an element or package to a new qualified name and rewrite every
//! qualified-name reference to it, atomically.
//!
...
/// A regex matching a qualified-name token (at least one `::` separator),
/// compiled once and reused across every rewrite call.
static QNAME_TOKEN_RE: std::sync::OnceLock<regex::Regex> = std::sync::OnceLock::new();
fn qname_token_re() -> &'static regex::Regex {
    QNAME_TOKEN_RE
        .get_or_init(|| regex::Regex::new(r"[A-Za-z0-9_]+(?:::[A-Za-z0-9_]+)+").unwrap())
}

/// Failure planning or applying a move.
...
pub enum MoveError {
    ...
    DestinationEqualsSource(String),
    #[error("Cannot move '{old}' into its own subtree ('{new}').")]
    DestinationInOwnSubtree { old: String, new: String },
    #[error("Destination already exists: {0}")]
    DestinationExists(String),
    #[error("Write failed for {path} ({io}); rolled back.")]
    WriteFailed {
        path: String,
        #[source]
        io: std::io::Error,
    },
    #[error("Cannot create destination directory {path} ({io}); rolled back.")]
    CreateDirFailed {
        path: String,
        #[source]
        io: std::io::Error,
    },
    #[error("Relocation failed ({io}); rolled back.")]
    RelocationFailed {
        #[source]
        io: std::io::Error,
    },
...
fn rewrite_qname_text(text: &str, old: &str, new: &str) -> Option<String> {
    let out = qname_token_re().replace_all(text, |caps: &regex::Captures| {
        let tok = &caps[0];
...
/// human-readable error on any planning/apply failure (the original state
...
pub fn move_element(
    ...
    if new == old {
        return Err(MoveError::DestinationEqualsSource(old));
    }
    if new.starts_with(&format!("{old}::")) {
        return Err(MoveError::DestinationInOwnSubtree { old, new });
    }
    ...
    let old_fs = if is_pkg {
        ...
        match src_file {
            Some(ref p) => p.clone(),
            None => return Err(MoveError::SourceHasNoFile(old)),
        }
    };
    ...
    for (p, orig, updated) in &edits {
        ...
        if let Err(e) = std::fs::write(p, updated) {
            rollback(&backups);
            return Err(MoveError::WriteFailed {
    ...
    if let Some(parent) = new_fs.parent() {
        if let Err(e) = std::fs::create_dir_all(parent) {
            rollback(&backups);
            return Err(MoveError::CreateDirFailed {
