...
# WASM foreign-format plugins (opt-in; see docs/model-guide/wasm-plugins.md)
./target/debug/syscribe -m model/ plugins run <alias> --dry-run      # invoke one plugin, print its raw envelope JSON — no merge
./target/debug/syscribe -m model/ validate --deny W530 --deny W532   # gate CI on plugin execution/envelope failures
```
...
WASM foreign-format plugins (`ADR-SYS-PLUGIN-001`; `crates/syscribe-model/src/plugins/`) let a package `_index.md` declare `foreignFormat: <alias>`, handing its subtree to a sandboxed TypeScript-authored WASM plugin (`extism`/`wasmtime`, compiled via `extism-js`) named by `[plugins.<alias>]` in `.syscribe.toml`. The plugin's `parse` export returns a JSON envelope of elements, merged into the graph the same way the FMEA/TARA row-explosion passes in `walker.rs` synthesize sibling elements — real, origin-agnostic `RawElement`s resolvable by every cross-reference kind. Read-only ingestion only (the foreign folder is never written back to); no WASI filesystem preopens — the plugin reads its subtree only through three scoped host functions (`fs_read`/`fs_list_dir`/`fs_exists`), each canonicalize-and-prefix-checked against the plugin's declared directory; no network. `wasm-plugins` is a `syscribe-model` Cargo feature, on by default in `syscribe`/`syscribe-server`. Config/envelope errors are `E530`–`E532`; execution/per-element failures are `W530`–`W534`, all degrading gracefully (never aborting the rest of `validate`). A generically-useful `E108` (duplicate qualified name, any origin) shipped alongside this feature. A plugin's output is cached at `.syscribe/cache/plugins.json`, keyed by a `blake3` hash of the `.wasm` module plus every file in its subtree (mirrors `syscribe summarize`'s `.syscribe/cache/summaries.json` convention) — only successful runs are cached, `plugins run --dry-run` always bypasses it. See `examples/wasm-plugins/sysmlv2-toy/` for a complete example plugin. **Not yet built** (tracked as follow-on scope): write-protection for plugin-owned elements in the mutate routes, and true fuzzing of the path-traversal boundary beyond the hand-written adversarial cases already covered.
